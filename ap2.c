...
char label[500], opcode[500], operand[500];
char symbol[500];
char value[100];
char mnemonic[500];
char operand_address[500];
char opcode_address[100];
char locctr[100];
int length = 0;
...
void get_length()
{
    FILE *fp4 = fopen("length.txt", "r");  //reading from length
    if(fp4== NULL)
...
int check_indexed()
{
    ...
    return is_indexed;   //index flag
}

int search_symtab()
{
	FILE *fp5=fopen("symtab.txt","r");   //reading from symtab
    if(fp5== NULL)
...
void pass2()
{
    ...
    while ((fgets(&line, &len, fp1)) != NULL)
    {
        ...
        if((!strcmp(label, " ")==0) || (!strcmp(opcode, " ")==0) || (!strcmp(operand, " ")==0))
        {
          ...
          /* else if (strcmp(label, "EOF") == 0)
            {
                   fprintf("454f46", object_code);
            }*/
            else
            {
                fprintf(fp2, "%s\t%s\t%s\t%s\n", locctr, label, opcode, operand);
